...
run_interactive() {
  ...
  local -a features=()
  if [[ ! -t 0 ]]; then
    echo "Interactive mode requires a TTY. Please run in a terminal." >&2
    exit 1
  fi
  temp_config=$(mktemp)
  ...
  read_keypress() {
    local __var="$1"
    if ! read -rsn1 "$__var" </dev/tty 2>/dev/null; then
      echo "Interactive input unavailable. Please run in a terminal." >&2
      exit 1
    fi
  }
  ...
  read_line() {
    local __var="$1"
    local __prompt="$2"
    if ! read -r -p "$__prompt" "$__var" </dev/tty 2>/dev/null; then
      echo "Interactive input unavailable. Please run in a terminal." >&2
      exit 1
    fi
  }
  ...
  while true; do
    ...
    if [[ "$dns_mode" == "custom" && -z "$dns_templates" ]]; then
      read_line dns_templates "Enter DoH template URL (required for custom mode): "
    fi
